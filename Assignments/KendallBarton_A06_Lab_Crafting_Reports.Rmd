...
```{r, echo=FALSE, fig.cap="Ozone by Month, 2018", fig.align='left', fig.height=3}
Oz_2018 <- epa_data %>%
  filter(Year == 2018) %>%
  drop_na(Ozone)

ggplot(Oz_2018, aes(x = factor(Month, levels = 1:12, labels = month.abb), y = Ozone)) +
  geom_boxplot() +
  labs(title = "2018 Ozone Levels by Month", y = expression(paste("Ozone (", mu, "g/",m^3,")")),
       x = "Month")

```
```{r, echo=FALSE, fig.cap="Ozone by Month, 2019", fig.align='left', fig.height=3}
Oz_2019 <- epa_data %>%
  filter(Year == 2019) %>%
  drop_na(Ozone)

ggplot(Oz_2019, aes(x = factor(Month, levels = 1:12, labels = month.abb), y = Ozone)) +
  geom_boxplot() +
  labs(title = "2019 Ozone Levels by Month", y = expression(paste("Ozone (", mu, "g/",m^3,")")),
       x = "Month")
```
...
[https://github.com/kamesbarton/EDE_Fall2023](https://github.com/kamesbarton/EDE_Fall2023)
