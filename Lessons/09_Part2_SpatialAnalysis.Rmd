...
mapView(huc8_sf, col.region = 'orange') +
  mapView(huc8_sf_un, col.region = 'purple')

ggplot() +
  geom_sf(data = huc8_sf, fill = "orange") + #| instead of + to have swiper
...
#Select intersecting counties using the `filter()` command (2nd way to do it)
neuse_intersect_2 <- counties_sf_utm %>% 
  filter(st_intersects(x = ., y = neuse_sf, sparse = FALSE))

the_filter <- st_intersects(x = counties_sf_utm, y = neuse_sf, sparse = F)
...
#Grab the intersecting HUC_8s, matrix selection or st
int_HUCs <- huc8_sf_utm %>%
  filter(tri_sf)
