...
NTL.TL.90s <- NTL.phys.data %>%
  filter(lakename == "Tuesday Lake") %>%
  filter(year4 %in% seq(1990, 1999))
dim(NTL.TL.90s)
```
...
NTL.TL.90s.Jul <- NTL.TL.90s %>%
  mutate(Month = month(as.Date(NTL.TL.90s$sampledate))) %>%
  filter(Month == 7)
...
#NTL.TL.90s.Jul %>%
mean_temp <- NTL.TL.90s.Jul %>%
  filter(!is.na(temperature_C) & depth == 0) %>%
...
NTL.irr.data <- NTL.phys.data %>%
  pivot_longer(irradianceWater & irradianceDeck, names_to = Irradiance_Type, Irradiance_Value, values_to = )
