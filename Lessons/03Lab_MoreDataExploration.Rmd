...
matrix1[4, ] #cols then rows
matrix1[ , 3] #
matrix1[c(12, 14)] #just get idx 12 and idx 14
matrix1[c(12:14)] #
matrix1[2:4, 1:3] #
...
```{r DataExercise-1a}
temp_f_high <- c(83, 83, 83, 78, 71, 79, 82, 85, 84, 82)
temp_f_low <- c(56, 58, 60, 61, 63, 63, 64, 64, 63, 60)
```
...
```{r DataExercise-1b}
f_to_c <- function(f_vec){
  return((f_vec - 32) * (5/9))
}

temp_c_high <- f_to_c (temp_f_high)
temp_c_low <- f_to_c (temp_f_low)
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
temp_df <- data.frame("High in Fahrenheit" = temp_f_high, "Low in Fahrenheit" = temp_f_low, "High in Celsius" = temp_c_high, "Low in Celsius" = temp_c_low)
```
...
```{r DataExercise-1d}
summary(temp_df)
summary(temp_df$High.in.Fahrenheit)
sd(temp_df$High.in.Celsius)
```
