...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text( #add text to plot
    x=0, #where text should be
    y=0.025,
    label=expression(paste("Mean is ", mu,",", R^{2},'=0.6')) #R^{2} for superscript! always use expression(paste()) but nesting not always needed
    )
...
#List unique values
unique(PeterPaul.chem.nutrients$month) #not all months represented
...
factor(PeterPaul.chem.nutrients$month, levels = 1:12, labels = month.abb) #abbreviated month format
class(PeterPaul.chem.nutrients$month)
...
#my_plot <- PeterPaul.chem.nutrients %>%
  #filter(depth ==0) %>%

ggplot(filter(PeterPaul.chem.nutrients, depth==0),
       aes(x = daynum, y = temperature_C, color = year4)) + #don't make year "as factor" for this plot
  geom_point() +
  facet_wrap(vars(lakename)) + #nrow to stack graphs
    labs(y="Surface Temper")
...
ggplot(PeterPaul.chem.nutrients,
  ...
  geom_point(size = .5) +
  scale_color_viridis(option = "magma", direction = -1) + #1 changes it to purple to orange
...
ggplot(EPAair,
       aes(x = PM2.5, y = Ozone, color = mean_Lat)) +
  geom_point(alpha = .5) +
  #scale_color_viridis(name = "Latitude", option = "viridis", direction = 1) + #optional color change
  geom_smooth(method = "lm") #linear reg line of best fit
