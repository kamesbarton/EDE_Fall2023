...
Rainfall_BR_processed <-
  ...
  mutate(Date = my(paste0(Month,"-",Year))) %>% #my = month/year
...
summary(BR_complete$Temperature_C) #ok way, but not best way
summary(BR_complete$Rainfall) #create a sequence of numbers or dates that you should have data fo
#use left join to join sequence to data, then use summary to check for NAs
...
temp_ts <- ts(BR_complete$Temperature_C, start = c(1901,1,1), frequency = 12) #start year/month/day; 1901/1/1 #repeating every 12 (monthly observations)
...
temp_decomp <- stl(temp_ts, s.window = "periodic") #default back to previously set frequency (12)
plot(temp_decomp) #gray boxes represent how much we had to zoom in (bigger gray bar, smaller impact on dataset)
...
temp_components <- mutate(temp_components, Observed = BR_complete$Temperature_C,
                          Date = BR_complete$Date)

rain_components <- as.data.frame(rain_decomp$time.series[,1:3])
rain_components <- mutate(temp_components, Observed = BR_complete$Rainfall,
                          Date = BR_complete$Date)
