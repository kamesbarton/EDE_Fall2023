...
myURL <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02087500&period=24&format=rdb'

gage_data <- read.table(myURL,skip = 30,header=TRUE,sep='\t',stringsAsFactors = T)
...
df1 <- data.frame(the_states,
                  as.numeric(gsub(",", "",the_price)),
                  as.numeric(gsub(",", "",the_capacity)),
                  as.numeric(gsub(",", "",net_generation)),
                  as.numeric(gsub(",", "",total_retail)))

```


```{r create.scrape.function}
column_list <- c(the_states, the_price, the_capacity, net_generation, total_retail)

scrape.it <- function(the_year){
  #Get the proper url
  df_return <- data.frame(c(seq(1,52)))
  the_url <- ifelse(
    the_year=='2022',
    'https://www.eia.gov/electricity/state/',
    paste0('https://www.eia.gov/electricity/state/archive/',the_year,'/'))
  get_web <- read_html(the_url)
  for (num in seq(1,5)) {
    this_data <- get_web %>%
      html_nodes(paste0("td:nth-child(",as.character(num),")")) %>%
      html_text()
    if (num != 1) {
      this_data <- as.numeric(gsub(",", "",this_data))
    }
    df_return <- cbind(df_return, this_data)
  }
  colnames(df_return) <- c("index", "the_states", "the_price", "the_capacity", "net_generation",
                           "total_retail")
  return(df_return)
}

# Use the above function to scrape data for 2017
electric_df.2017 <- scrape.it(2017)
