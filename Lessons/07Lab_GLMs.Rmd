...
ggplot(EPAair, aes(x = Ozone, fill = as.factor(Year))) +
  ...
  labs(fill="") #make fill "Year" to add legend title

ggplot(EPAair, aes(y = Ozone, x = as.factor(Year))) +
  geom_boxplot() +
  #make letters represent the max values
  stat_summary(geom = "text", fun = max, vjust= -1, size = 4, label = c("b", "a")) +
  labs(y = "Ozone AQI Value", x= "") +
  ylim(0,150)

#bowplot gives quartiles, but not distribution and density like violin does
ggplot(EPAair, aes(y = Ozone, x = as.factor(Year))) +
...
#consider including LAT or maybe MONTH as correlations
...
PM2.5_reg2 <- lm(data = EPAair.subset, PM2.5 ~ Ozone + Month + mean_Lng + mean_Lat)
summary(PM2.5_reg2)
